...
def test_profitable_harvest(accounts, token, vault, strategy, strategist, amount, rook_whale, rook):
    ...
    strategyBreakdown(strategy)
    genericStateOfVault(vault, token)
    strategy.testSwap(10000, {'from': strategy.address})
...
#
#
# def test_triggers(gov, vault, strategy, token, amount, weth, weth_amout, rook, rook_whale):
#     # Deposit to the vault and harvest
#     token.approve(vault.address, amount, {"from": gov})
#     vault.deposit(amount, {"from": gov})
#     vault.updateStrategyDebtRatio(strategy.address, 5_000, {"from": gov})
...
#     rook.transfer(strategy.address, 1 * 10 ** 10, {"from": rook_whale})
#     assert strategy.harvestTrigger(0) == False
#
